...
# 色彩拟合 绿
green_arr = ["#e6fcf5", "#c3fae8", "#96f2d7", "#63e6be",  # Teal
             "#38d9a9", "#20c997", "#12b886", "#0ca678", "#099268", "#087f5b",
             "#ebfbee", "#d3f9d8", "#b2f2bb", "#8ce99a",  # Green
             "#69db7c", "#51cf66", "#40c057", "#37b24d", "#2f9e44", "#2b8a3e",
             "#f4fce3", "#e9fac8", "#d8f5a2", "#c0eb75",  # Lime
             "#a9e34b", "#94d82d", "#82c91e", "#74b816", "#66a80f", "#5c940d"]
# 色彩拟合sec
colown_arr = ["#e3fafc", "#c5f6fa", "#99e9f2", "#66d9e8",  # Cyan
              "#3bc9db", "#22b8cf", "#15aabf", "#1098ad", "#0c8599", "#0b7285",
              "#e6fcf5", "#c3fae8", "#96f2d7", "#63e6be",  # Teal
              "#38d9a9", "#20c997", "#12b886", "#0ca678", "#099268", "#087f5b",
              "#ebfbee", "#d3f9d8", "#b2f2bb", "#8ce99a",  # Green
...
def cl_color(r, g, b):  # 颜色拟合
    ...
    for i in color_arr:
        ...
        dis = math.sqrt(pow(i_h-h, 2)+pow((i_s-s)*10, 2)+pow((i_v-v)*20, 2))
        if dis < min:
            min = dis
            cl_color = i
    return cl_color
...
def clown_color(r, g, b):  # 颜色拟合 自定义颜色
    min = 1000
    cl_color = ""
    h, s, v = rgb2hsv(r, g, b)
    for i in colown_arr:
...
def cl_green(r, g, b):  # 绿色算法
    ...
    for i in green_arr:
        ...
        ib = int(i[5:7], 16)
        i_h, i_s, i_v = rgb2hsv(ir, ig, ib)
        dis = math.sqrt(pow(i_h-h, 2)+pow((i_s-s)*15, 2)+pow((i_v-v)*15, 2))
        if dis < min:
            min = dis
            cl_color = i
    return cl_color
...
for i in range(0, 120):
    for j in range(0, 122):
        ...
        color_str = "#"+'00' + \
                    ...
                    '00'
        '''
        color_str = cl_green(foldr[i][j], foldg[i][j], foldb[i][j])
        file.write('<rect x="'+str((j+1)*magrin+j*width)+'" y="'+str((i+1)*magrin+i*height) +
                   '" width="'+str(width)+'" height="'+str(height)+'" fill="'+color_str+'" />')
file.write('</svg>')
...
# 色彩拟合
file = open('output/ownclo_final.svg', 'w')
file.write('<svg version="1.1" baseProfile="full" width="8000" height="8000" xmlns="http://www.w3.org/2000/svg">')
for i in range(0, 120):
    for j in range(0, 122):
        color_str = clown_color(foldr[i][j], foldg[i][j], foldb[i][j])
        file.write('<rect x="'+str((j+1)*magrin+j*width)+'" y="'+str((i+1)*magrin+i*height) +
                   '" width="'+str(width)+'" height="'+str(height)+'" fill="'+color_str+'" />')
file.write('</svg>')
